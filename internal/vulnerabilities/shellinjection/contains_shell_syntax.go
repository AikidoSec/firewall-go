...
func buildCommandsRegex() *regexp.Regexp {
	...
	for i, prefix := range pathPrefixes {
		prefixes[i] = escapeStringRegexp(prefix)
	}
...
func containsShellSyntax(command, userInput string) bool {
	...
	for _, match := range matchAll(command, commandsRegex) {
		...
		if slices.Contains(separators, charBefore) && slices.Contains(separators, charAfter) {
			return true
		}

		// e.g. <separator>command
		if slices.Contains(separators, charBefore) && charAfter == "" {
			return true
		}

		// e.g. command<separator>
		if charBefore == "" && slices.Contains(separators, charAfter) {
			return true
		}
	}
