...
func OnRequestShutdownReporting(method string, route string, statusCode int, user string, ip string, rateLimitGroup string, apiSpec *aikido_types.APISpec) {
	...
	agent.OnRequestShutdown(method, route, statusCode, user, ip, rateLimitGroup, apiSpec)
}
...
func OnPostRequest(ctx context.Context, statusCode int) {
	...
	go OnRequestShutdownReporting(
		reqCtx.Method, reqCtx.Route, statusCode,
		reqCtx.GetUserID(), reqCtx.GetIP(), reqCtx.GetRateLimitGroup(),
		apiSpec,
	)
}
