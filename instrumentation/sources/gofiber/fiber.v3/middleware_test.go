...
// newTestApp trusts the fixed remote address app.Test() uses (0.0.0.0) so
// tests can control c.IP() via the X-Forwarded-For header: app.Test() serves
// requests over an in-memory connection whose RemoteAddr is always fixed, so
// RemoteAddr on the request has no effect on the IP fiber observes.
...
func TestMiddlewareReportsPlainErrorStatusCode(t *testing.T) {
	agent.State().GetRoutesAndClear()
	...
	assert.Never(t, func() bool {
		return len(agent.State().GetRoutesAndClear()) > 0
	}, 100*time.Millisecond, 10*time.Millisecond, "a 500 route should not be discovered")
}
